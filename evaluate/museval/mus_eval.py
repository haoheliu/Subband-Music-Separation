...
def bss_eval(reference_sources, estimated_sources,
  ...
  .. [#vincent2005bssevalv3] Emmanuel Vincent, Rémi Gribonval, and Cédric
    ...
    def proc(src):
        return src #(np.nanmedian(src[0]),np.nanmedian(src[1]))
    ...
    return (proc(result[SDR]), proc(result[ISR]), proc(result[SIR]), proc(result[SAR]), popt)
...
def read_wav(estimate_fname,target_fname):
    ...
    estimate, truth = estimate[:min_length].reshape((1, min_length, 1)), truth[:min_length].reshape((1, min_length, 1))
...
if __name__ == "__main__":
    ...
    bac_estimate_fname = "/home/work_nfs/hhliu/workspace/github/internship_music_separation/outputs/musdb_test/2_2020_3_22__unet_spleeter_spleeter_sf0_l1_l2_l3_lr0001_bs2-48_fl3_ss32000_85lnu5mu0.5sig0.2low0.3hig0.5fshift8flength32drop0.5split_bandFalse192000/background_test_0.wav"
    bac_target_fname = "/home/work_nfs/hhliu/workspace/datasets/musdb18hq/test/test_0/background.wav"

    voc_estimate_fname = "/home/work_nfs/hhliu/workspace/github/internship_music_separation/outputs/musdb_test/2_2020_3_22__unet_spleeter_spleeter_sf0_l1_l2_l3_lr0001_bs2-48_fl3_ss32000_85lnu5mu0.5sig0.2low0.3hig0.5fshift8flength32drop0.5split_bandFalse192000/vocal_test_0.wav"
    voc_target_fname = "/home/work_nfs/hhliu/workspace/datasets/musdb18hq/test/test_0/vocals.wav"

    voc_estimate,voc_target = read_wav(voc_estimate_fname,voc_target_fname)
    bac_estimate,bac_target = read_wav(bac_estimate_fname,bac_target_fname)

    estimate = np.concatenate((voc_estimate,bac_estimate),axis=0)
    target = np.concatenate((voc_target,bac_target),axis=0)
